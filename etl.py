...
def process_song_data(spark, input_data, output_data):
    ...
    artists_table = artists_table.write.parquet(os.path.join(output_data,'artists/artists.parquet'),'overwrite')
...
def process_log_data(spark, input_data, output_data):
    ...
    actions_df = df.select( 'ts', 'userId', 'level', 'song', 'artist', 'sessionId', 'location', 'userAgent')\
    ...
    users_table = users_table.write.parquet(os.path.join(output_data, 'users/users.parquet'), 'overwrite')
    ...
    actions_df = songplays_table.withColumn('datetime', get_datetime(actions_df.ts))
    ...
    songplays_table =  actions_df.join(song_df, actions_df.song == song_df.title,how='left')
...
def main():
    ...
    output_data = "s3a://udacity-data-10/"
